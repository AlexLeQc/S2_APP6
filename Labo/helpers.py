...
def bodeplot(b, a, title):
    ...
    ph = np.unwrap(np.angle(h))  # calcul du déphasage en radians
...
def simplifytf(z, p, k):
    ...
    while p.any() and z.any():  # tant que le numérateur et le dénominateur contiennent encore des racines
        match = False
    ...
    for i, zval in enumerate(z):
        rez = np.real(zval)
        if np.abs(rez - np.round(rez)) < tol:   # teste si la valeur est identique à un entier
            rez = np.round(rez)
        imz = np.imag(zval)
        if np.abs(imz - np.round(imz)) < tol:
            imz = np.round(imz)
        z[i] = rez + imz*1j
    for i, pval in enumerate(p):
        rep = np.real(pval)
        if np.abs(rep - np.round(rep)) < tol:
            rep = np.round(rep)
        imp = np.imag(pval)
        if np.abs(imp - np.round(imp)) < tol:
            imp = np.round(imp)
        p[i] = rep + imp*1j
    if np.abs(k - np.round(k)) < tol:
        k = np.round(k)
    return z, p, k
